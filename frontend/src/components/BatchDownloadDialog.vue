<template>
  <el-dialog :model-value="visible" @update:model-value="$emit('update:visible', $event)"
    ...
    <el-alert type="info" :closable="false" style="margin-bottom:12px">
      ...
      选「全部模型」时该行的品类/题区间应用到所有模型，创建批次时展开成每模型一个 unit。
      同一模型可分多次创建不同品类/区间（如 豆包·海外云主机·1-12，再 豆包·海外云主机·13-20），
      导入后按 (任务,模型,问题) 自动合并，不会覆盖已导入结果。
      <br/>模型不要求预先登录：未登录的模型首次运行时会弹浏览器让你登录，登录态自动保存复用。
      <br/>创建批次后配置会自动推送给 Win 守护进程，无需下载到本地。
    </el-alert>
    ...
    <template #footer>
      ...
      <el-button type="success" :loading="submitting" @click="submitBatch">确定并创建批次</el-button>
    </template>
  </el-dialog>
</template>
...
async function downloadBatch() {
  ...
  try {
    ...
    if (!res?.success) return ElMessage.error(res?.detail || '创建批次失败')
    // 不再下载配置文件到浏览器：批次已落库，并已 webhook 推送给 Win 守护进程。
    // 状态会从 pushed → awaiting_human → running → imported 自动流转，
    // 父组件监听 @downloaded 后启动轮询刷新，无需人工下载/刷新。
    ElMessage.success('批次已创建并推送至守护进程，状态将自动更新')
    emit('downloaded')
    emit('update:visible', false)
  } catch (e) {
    ElMessage.error(`创建批次失败: ${e.message || e}`)
